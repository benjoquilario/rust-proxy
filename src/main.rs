use actix_web::{
    get, http::header, http::Method, middleware::Compress, web::Bytes, App, HttpRequest,
...
#[inline]
fn process_m3u8_line(line: &str, scrape_url: &Url, headers_param: &Option<String>) -> String {
    if line.is_empty() {
        return String::new();
    }
        ...
        if lines.len() > 1000 {
            ...
            for chunk_result in processed_chunks {
                match chunk_result {
                    ...
                    Err(_) => {
                        return HttpResponse::InternalServerError().body("M3U8 processing failed")
                    }
                }
            }
        } else {
...
async fn main() -> std::io::Result<()> {
    ...
    HttpServer::new(|| {
        App::new()
            ...
            .route(
                "/",
                actix_web::web::method(Method::OPTIONS).to(handle_options),
            )
    })
